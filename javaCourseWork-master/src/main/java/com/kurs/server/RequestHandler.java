    ...
    private void startProcessing() {
        try {
            ...
            switch (request.getRequestID()) {
                ...
                case 7 -> {
                    serverLogger.info("Пришёл запрос на подтверждение блюда");
                    new ChangeVisibilityForDish(request);
                    sendResponse(new Gson().toJson(new Response(200)));
                }
                case 8 -> {
                    serverLogger.info("Пришёл запрос на очистку меню :(");
                    new ClearDB();
                    sendResponse(new Gson().toJson(new Response(200)));
                }
            }
        } catch (IOException e) {
            serverLogger.error("Пришёл какой-то странный запрос" + e.getMessage());
        }
    }
